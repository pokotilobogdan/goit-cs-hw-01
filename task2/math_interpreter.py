...
class Lexer:
    ...
    def integer(self):
        ...
        while self.current_char is not None and self.current_char.isdigit():
            result += self.current_char     # concatenation, not sum
            self.advance()
        return int(result)
...
class BinOp(AST):
    ...
    def __str__(self):
        return f'BinOp({self.left}{repr(self.op)}{self.right})'
...
        """
    ...
    def term(self):         # term = доданок
        """ Парсер для 'term' правил граматики. У нашому випадку - це цілі числа."""
        
        while self.current_token.type not in (TokenType.PLUS, TokenType.MINUS, TokenType.RPAREN, TokenType.EOF):
    ...
    def expr(self):
        ...
        Повертає дерево арифметичних операцій.
        """
        node = self.term()
